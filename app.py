...
# --- DOCX Builder: Official Lesson Plans ---
...
with st.sidebar:
    ...
    app_mode = st.radio(
        "Select Tool Mode:",
        ["📋 Lesson Plan Generator", "🌟 A* Comprehensive Lesson Notes"],
        index=0
    )
    st.markdown("---")
    ...
    if st.button("💾 Save Key to this Device"):
        if user_key.strip():
            ...
            st.info("Key cleared.")
    st.markdown("---")
    st.markdown("**Get a Free Key (100% Free):**\n1. Open [aistudio.google.com](https://aistudio.google.com)\n2. Click **Get API key**\n3. Create in a new project and paste here.")

api_key = user_key.strip()
if not api_key:
    st.warning("👈 Please enter your free Gemini API key in the left sidebar to activate the generator.")
    st.stop()

# =======================================================
# MODE 1: INSPECTORATE LESSON PLAN GENERATOR
# =======================================================
if app_mode == "📋 Lesson Plan Generator":
    ...
    if st.button("Generate Inspection Plans", type="primary"):
        ...
        if not scheme_detail.strip() and not extracted_scheme and not extracted_curriculum:
            ...
            with st.spinner(f"Parsing curriculum data for {term_selected} - {week_selected} and generating {lesson_count} lesson plan(s)..."):
                ...
                2. LESSON OBJECTIVES: Active Bloom's Taxonomy verbs citing matching framework code (e.g. [CHE1.1.1]).
                ...
                4. INSTRUCTIONAL RESOURCES & WORKING URL LINKS:
                   - Clickable direct YouTube search URL: "YouTube Video: [Title] - [https://www.youtube.com/results?search_query=](https://www.youtube.com/results?search_query=)[encoded+terms]"
                   - Clickable PhET simulation URL: "PhET Simulation: [Title] - [https://phet.colorado.edu/en/simulations/filter?subjects=](https://phet.colorado.edu/en/simulations/filter?subjects=)[topic]&type=html"
                ...
                try:
                    ...
                    st.success(f"Generated {len(data)} inspection lesson plan(s) successfully!")
                    st.download_button(
                        label="📥 Download Inspection Word Document (.docx)",
                        data=docx_bytes,
                        file_name=f"NGA_{term_selected.replace(' ', '_
git add app.py
git commit -m "Restore complete Lesson Plan generator interface"
git push origin main
cd ~/Desktop/Lesson_Plan_Maker
cat << 'EOF' > app.py
import streamlit as st
import json
import io
...
if st.session_state.plans_output:
    st.success(f"Lesson plan document ready!")
    st.download_button(
        label="📥 Download Inspection Word Document (.docx)",
